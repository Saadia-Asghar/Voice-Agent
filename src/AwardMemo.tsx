...
export function AwardMemo({ downtime }: { downtime: number }) {
  ...
  return <main className="workflow-screen memo-screen">
    ...
    <div className="memo-grid">
      ...
      <section className="workflow-panel"><span className="eyebrow">Human approval boundary</span><h2>Review before award.</h2><p>ScopeDial recommends; it cannot purchase, accept a quote, or bind City Labs.</p><label className="approval-check"><input type="checkbox" /> I reviewed the assumptions and evidence.</label><button className="primary-button" disabled>Authorize next step</button><small className="human-note"><TriangleAlert size={14} /> Demo mode intentionally prevents a binding action.</small></section>
    </div>
