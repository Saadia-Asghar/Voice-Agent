/**
 * Demo video: UX screen recording + voiceover + burned subtitles.
 */
import { createRequire } from "node:module";
import { spawnSync } from "node:child_process";
import { mkdirSync, readdirSync, copyFileSync, existsSync, writeFileSync } from "node:fs";
...
const ASS = join(OUT, "demo-live.ass");
...
if (!existsSync(VOICE)) throw new Error("Missing demo-voice.wav");

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

const CHAPTERS = [
  { t: 0, badge: "DEMO · UX FLOW", line: "One brief · three calls · one human award" },
  { t: 7, badge: "01 ESTIMATOR", line: "Upload report · resolve conflicts · lock ScopePrint" },
  { t: 20, badge: "02 CALLER", line: "Three styles · identical scope · structured outcomes" },
  { t: 32, badge: "03 CLOSER", line: "Compare · 30% red-flag · concession ledger" },
  { t: 44, badge: "AWARD MEMO", line: "Ranked recommend with transcript receipts" },
];
...
function writeAss() {
  ...
  const lines = [
    "[Script Info]", "Title: BenchDial Demo", "ScriptType: v4.00+", "PlayResX: 1440", "PlayResY: 900", "",
    "[V4+ Styles]",
  ...
  for (let i = 0; i < CHAPTERS.length; i++) {
    ...
    const end = i + 1 < CHAPTERS.length ? CHAPTERS[i + 1].t : 56;
...
async function prepareScope(page) {
  ...
  await click(page, 'button:has-text("Lock confirmed scope")', 800);
}
